...
func main() {
	...
	cleanWindow, err := fetchRealHistory(binanceClient, Symbol, Interval, VectorWindow+5)
	if err != nil {
	...
	wsHandler := func(event *futures.WsKlineEvent) {
		...
		if len(matches) > 0 {
			...
			_, err_ := llmClient.GenerateSignal(context.Background(), sysMsg, usrMsg, b64A, b64B)
			if err_ != nil {
				logger.Info(fmt.Sprintf("LLM Error: %v", err_))
				return
			}
