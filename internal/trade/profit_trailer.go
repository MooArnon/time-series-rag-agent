...
// CalculateTakeProfitPrice returns the take-profit price for a leveraged position.
// percentageTp is the desired profit percentage relative to the notional value.
// positionDir must be either "LONG" or "SHORT".
func CalculateTakeProfitPrice(leverage int, percentageTp float64, positionDir string, price float64) (float64, error) {
    if leverage <= 0 {
